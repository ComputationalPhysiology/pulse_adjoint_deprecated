...
def load_data():
    ...
    dolfin.File('cfun.pvd') << geometry.cfun
...
def forward(geometry, matparams, lvps, gammas, fname='forward_results.h5'):
    ...
    crl_basis = dict(
        circumferential=geometry.c0,
        radial=geometry.r0,
        longitudinal=geometry.l0
    )
    straintarget = \
        pulse_adjoint.optimization_targets.RegionalStrainTarget(
            geometry.mesh,
            crl_basis,
            geometry.dx,
            tensor="E",
    ...
    for u in us:
        ...
        for region in range(nregions):
            strains['region_{}'.format(region+1)].append(straintarget.simulated_fun[region].vector().get_local())
...
def plot_data(data):
    ...
    for name, ax in zip(['circumferential', 'radial',
                         'longitudinal'],
                        [ax_circ, ax_rad, ax_long]):
    ...
    for i, (ax, name) in enumerate(zip([ax_circ, ax_rad, ax_long],
                                       ['circumferential', 'radial',
                                        'longitudinal'])):
        for j, strain in enumerate(data['strains'].values()):
            ax.flatten()[j].plot(np.transpose(strain)[i])

    for name, fig in zip(['pv_loop', 'circumferential_strain',
                          'radial_strain', 'longitudinal_strain'],
