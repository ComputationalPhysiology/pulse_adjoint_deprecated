...
class MyCallBack(object):
    ...
    def __call__(self, x):
        ...
        grad_norm = (
            None if len(self.rd.grad_norm_scaled) == 0 else self.rd.grad_norm_scaled[-1]
        )
        ...
        logger.info(print_line(self.rd.for_res, self.ncalls, grad_norm, func_value))
    ...
    def _get_options(self, lb, ub, tol, max_iter, **kwargs):
        ...
        if module == "scipy":
            ...
            self.options = get_scipy_options(
                method, self.rd, lb, ub, tol, max_iter, **kwargs
            )
            ...
            self.problem, self.options = get_pyOpt_options(
                self.rd, lb, ub, tol, max_iter, **kwargs
            )
