...
def load_measured_strain(d, patient, key="measured_strain"):
    ...
    for k in list(d[key].keys()):
        d[key][k] = {r: {} for r in list(patient.strain.keys())}

    for k1, v1 in patient.strain.items():
...
def get_relative_paths(params):
    ...
    for path_key in ["echo_path", "mesh_path", "pressure_path"]:
        ...
        if not path_found:
            ...
            if not path_found:
                msg = "Path {} not found. ".format(
                    path_key
                ) + "Pleas specify this path in the parameter file." "\nDefault path is {}".format(
                    path
                )
                raise IOError(msg)
...
def load_parameters(fname, key=None):
    ...
    if key in d:
        ...
        msg = "Parameters does not have key {}. Possible keys are".format(
            key, list(d.keys())
        )
        raise KeyError(msg)
...
def get_value_from_h5dict(d):
    ...
    if isinstance(d, np.ndarray):
        ...
        if len(list(d.keys())) == 0:
            ...
            if "vector_0" in list(d.keys()):
                return d["vector_0"]

            else:
                msg = "Do not know what to return" + "Possible keys are {}".format(
                    list(d.keys())
                )
...
def h5dict_to_list(d):
    ...
    keys = sorted(list(d.keys()), key=lambda t: int(t))
    lst = []
    for k in keys:
...
def get_unloaded_data(params):
    ...
    keys = sorted(
        [a for a in list(all_data.keys()) if a.isdigit()], key=lambda t: int(t)
    )
    ...
    for k in keys[:-1]:
        ...
        target = np.array(
            [
                a[0]
                for a in h5dict_to_list(
                    all_data[k]["passive_inflation"]["volume"]["target"]
                )
            ]
        )
        simulated = np.array(
            [
                a[0]
                for a in h5dict_to_list(
                    all_data[k]["passive_inflation"]["volume"]["simulated"]
                )
            ]
        )
        ...
        optimal_control = all_data[k]["passive_inflation"]["optimal_control"][
            "vector_0"
        ]
...
def save_dict_to_h5(d, h5name, h5group="", overwrite_file=True, overwrite_group=True):
...
def get_data(params, patient=None):
    ...
    passive = (
        {} if "passive_inflation" not in all_data else all_data["passive_inflation"]
    )
    active = (
        {} if "active_contraction" not in all_data else all_data["active_contraction"]
    )
    active_keys = sorted(
        list(active.keys()), key=lambda t: int(t.rsplit("contract_point_")[-1])
    )
    ...
    if patient is None:
        patient = get_patient_geometry_from_results(params, "unloaded" in all_data)
    ...
    if npassive == 1:
        ...
        paramvec = dolfin.Function(
            dolfin.VectorFunctionSpace(patient.mesh, "R", 0, dim=npassive),
            name="matparam vector",
        )
    ...
    if params["unload"]:
        ...
        for k in list(all_data.keys()):
            if k.isdigit():
                if "passive_inflation" in all_data[k]:
        ...
        if not unload_iters or "passive_inflation" not in all_data[unload_iters[0]]:
            return {}, patient

        data["unload"]["target_volumes"] = all_data[unload_iters[0]][
            "passive_inflation"
        ]["volume"]["target"]
        if rv:
            data["unload"]["target_rv_volumes"] = all_data[unload_iters[0]][
                "passive_inflation"
            ]["rv_volume"]["target"]
        ...
        for k in unload_iters:
            its = []
            for i in list(all_data[k].keys()):
                if i.isdigit():
                    its.append(i)
            unload_subiters[k] = sorted(its, key=lambda t: int(t))

            try:
                data["unload"]["func_vals"][k] = all_data[k]["passive_inflation"][
                    "optimization_results"
                ]["func_vals"]
            except:
                pass
            data["unload"]["optimized_volumes"][k] = all_data[k]["passive_inflation"][
                "volume"
            ]["simulated"]
            if rv:
                data["unload"]["optimized_rv_volumes"][k] = all_data[k][
                    "passive_inflation"
                ]["rv_volume"]["simulated"]
        ...
        with dolfin.HDF5File(
            ...
            def get_reference_volume(k, marker):
                h5file.read(
                    refmesh, "/".join([k, "unloaded", "geometry", "mesh"]), True
                )
            ...
            for k in unload_iters:
                ...
                print(mat)
                try:
                    data["unload"]["material_parameters"][k] = mat
                except:
                    pass

                try:
                    data["unload"]["backward_displacement"][
                        k
                    ] = get_backward_displacement(k)
                except:
                    pass
                ...
                if rv:
                    try:
                        data["unload"]["reference_rv_volume"][k] = get_reference_volume(
                            k, 20
                        )
                    except:
                        pass
    ...
    if params["unload"]:
        ...
        measured_pressure = np.subtract(patient.pressure, patient.pressure[0])
    ...
    with h5py.File(params["sim_file"], "r") as h5file:
        ...
        if "optimization_results" in h5file[passive_group]:
            for k in list(opt_res.keys()):

                if k in h5file[passive_group]["optimization_results"]:
                    data["passive_optimization_results"][k] = h5file[passive_group][
                        "optimization_results"
                    ][k][:]
        ...
        for p in range(len(active_keys)):
            ...
            if active_group.format(p) in h5file:
                ...
                for k in list(opt_res.keys()):
                    opt_res_[k] = h5file[active_group.format(p)][
                        "optimization_results"
                    ][k][:]

                data["active_optimization_results"][
                    "contract_point_{}".format(p)
                ] = opt_res_
    ...
    with dolfin.HDF5File(dolfin.mpi_comm_world(), params["sim_file"], "r") as h5file:
        ...
        # Material parameters
        mat = read_matparam(
            h5file, "/".join([passive_group, "optimal_control"]), paramvec
        )
        print(mat)
        ...
        N = (
            ...
            if params["unload"]
            else patient.passive_filling_duration
        )
        ...
        for p in range(len(active_keys)):
            ...
            if not it in interpolation_points:
                ...
                h5file.read(
                    gamma, "/".join([active_group.format(p), "optimal_control"])
                )
...
def get_kwargs(patient, params):
    ...
    kwargs["dx"] = Measure("dx", subdomain_data=patient.sfun, domain=patient.mesh)
...
def load_measured_strain_and_volume(patient, params, num_points=None):
    ...
    if hasattr(patient, "time"):
        ...
        time_stamps = list(range(num_points))
