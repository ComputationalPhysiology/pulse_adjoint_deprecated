...
def get_colormap(ncolors, transparent=False):
    ...
    if transparent:
        ...
        if ncolors == 1:
            ...
            return [sns.xkcd_rgb["pale red"], sns.xkcd_rgb["denim blue"]]
...
def plot_gamma_mean_std(means, stds, path, labels=None, valve_times=None):
    ...
    if valve_times is not None:
        ...
        vals = [(a - pfb) % n for a in list(echo_valve_times.values())]
        keys = [k for k in list(echo_valve_times.keys())]
        ax.set_xticks(vals)
...
def plot_multiple_gamma(gammas, path, labels=None, valve_times=None):
    ...
    linestyles = lineStyles * (int(m / 3.0) + 1)
    fig = plt.figure()
    ax = fig.gca()
    ...
    for i, g in enumerate(gammas):
        ...
        ax.plot(x, g, label=labels[i], color=colors[i], linestyle=linestyles[i])
...
def plot_single_regional_gamma(gammas, path, valve_times=None, include_global=False):
    ...
    fig.savefig(path, bbox_extra_artists=(lgd,), bbox_inches="tight", format="png")
...
def plot_pv_loop_w_elastance(vs_sim, vs_meas, pressures, es, v0s, ES, path, labels):
    ...
    fig = plt.figure()
    ax = fig.gca()
    for i, (v_sim, v_meas, pressure, e, v0) in enumerate(
        zip(vs_sim, vs_meas, pressures, es, v0s)
    ):

        if i == 0:
            ax.plot(
                [v0[ES[i]], v_sim[ES[i]]], [0, pressure[ES[i]]], "g-", label="ESPVR"
            )
        else:
            # No label
...
def plot_single_strain_curves(simulated, measured, path, groups=None, unload=False):
    ...
    colors = get_colormap(len(list(simulated.keys())))
    linestyles = ["s", "^"]
...
def plot_strains(
    ...
    if dirs[0] not in simulated_strains:
    ...
    nregions = (
        len(list(simulated_strains[dirs[0]].keys())) if nregions is None else nregions
    )
    ...
    if nregions == 16:
        regions_sep = [[1, 3, 4, 6, 5, 2], [7, 9, 10, 12, 11, 8], list(range(13, 17))]
        grid = list(range(1, 22))

    elif nregions == 18:
        regions_sep = [[1, 3, 4, 6, 5, 2], [7, 9, 10, 12, 11, 8], list(range(13, 19))]
        grid = list(range(1, 22))

    elif nregions == 12:
        regions_sep = [[1, 3, 4, 6, 5, 2], [7, 9, 10, 12, 11, 8]]
        small_labels = [r"Basal", r"Mid"]
        grid = list(range(1, 15))
        regions.pop(7)

    else:  # nregions = 17
        regions_sep = [[1, 3, 4, 6, 5, 2], [7, 9, 10, 12, 11, 8], list(range(13, 18))]
        grid = list(range(1, 22))
    ...
    for d in range(len(dirs)):
        ...
        for i in range(len(regions_sep)):
            ...
            for region in regions_sep[i]:
                ...
                sim_region = (
                    str(region)
                    if isinstance(list(simulated_strains[direction].keys())[0], str)
                    else region
                )
                meas_region = (
                    str(region)
                    if isinstance(list(measured_strains[direction].keys())[0], str)
                    else region
                )

                if unload:
                    cur_strain = [
                        measured_strains[direction][meas_region],
                ...
                min_strain = np.min(
                    [
                        np.min(measured_strains[direction][meas_region]),
                        np.min(simulated_strains[direction][sim_region]),
                    ]
                )
                max_strain = np.max(
                    [
                        np.max(measured_strains[direction][meas_region]),
...
def plot_strain_scatter(data, path, labels=None, split=0):
    ...
    if split == 0:
        ...
        assert np.all(np.sort(list(data.keys())) == np.sort(["simulated", "measured"]))

        ax.scatter(data["measured"], data["simulated"], color=color)
        smin = np.min([np.min(data["simulated"]), np.min(data["measured"])])
        smax = np.max([np.max(data["simulated"]), np.max(data["measured"])])
        ...
        labels = (
            {"passive": "passive", "active": "active"} if labels is None else labels
        )
        ...
        for i, (k, v) in enumerate(fits.items()):
            ...
            ax.plot(x, v[0] * x + v[1], color="k", linestyle=lineStyles[i], label=label)
...
def plot_cardiac_work(work, labels, measured_work, path):
    ...
    for i, w in enumerate(work):
        ...
        ax.plot(x, w, label=labels[i], color=colors[i], linestyle=linestyles[i])
...
def plot_geometric_distance(
    dist, labels, path, valve_times=None, groups=None, stds=None
):

    setup_plot()

    colors = get_colormap(len(dist))
    color_group = None if groups is None else get_colormap(len(list(groups.keys())))
    ...
    if groups:
        dist_groups = [[] for i in range(len(list(groups.keys())))]
        labels_groups = list(groups.keys())
        ...
        for i, d in enumerate(dist):
            ...
            if stds is not None:
                ax.fill_between(
                    x, d + stds[i], d - stds[i], facecolor=colors[i], alpha=0.5
                )
