...
class PostProcess(object):
    ...
    def plot_pv_loop(self, single=True, groups=None, chamber="lv"):
        ...
        if len(keys) == 0 or "simulated_volume" not in self._results[keys[0]]:
            self.compute("volume")
        ...
        if single:
            ...
            for patient_name in list(self._results.keys()):
                ...
                plot.plot_single_pv_loop(
                    v_sim, v_meas, pressure, path, params["unload"]
                )

                if "measured_volume_rv" in self._results[patient_name]:
    ...
    def plot_pv_loop_w_elastance(self, single=True, groups=None):
        ...
        if len(keys) == 0 or "elastance" not in self._results[keys[0]]:
        ...
        if single:
            ...
            for patient_name in list(self._results.keys()):
                ...
                plot.plot_pv_loop_w_elastance(
                    [v_sim], [v_meas], [pressure], [es], [v0s], [ES], path, [""]
                )
            ...
            path = "/".join([outdir, "all_loops"])
            plot.plot_pv_loop_w_elastance(
                vs_sim, vs_meas, pressures, es, v0s, ES, path, labels
            )
    ...
    def plot_strain_curves(self, region=1, component="longitudinal", groups=None):
        ...
        keys = list(self._results.keys())
        if len(keys) == 0 or "simulated_strain" not in self._results[keys[0]]:
            self.compute("strain")
        ...
        for patient_name in list(self._results.keys()):
            ...
            simulated[patient_name] = self._results[patient_name]["simulated_strain"][
                component
            ][region]

            measured[patient_name] = self._results[patient_name]["measured_strain"][
                component
            ][region]

        path = "/".join([outdir, "strain_curves"])
        plot.plot_single_strain_curves(
            simulated, measured, path, groups, self._params["unload"]
        )

    def plot_strain(
        self, dirs=["circumferential", "radial", "longitudinal"], nregions=17
    ):
        ...
        for patient_name in list(self._results.keys()):
            ...
            paths = plot.plot_strains(
                simulated, measured, outdir, dirs, nregions, params["unload"]
            )
    ...
    def plot_strain_scatter(self, split=1, active_only=False, unload=True):
        ...
        strain_comp = {"longitudinal": [], "circumferential": [], "radial": []}
        data = {
            "simulated_active": deepcopy(strain_comp),
        ...
        for patient_name in list(self._results.keys()):
            ...
            for direction in list(sim.keys()):
                for region in list(sim[direction].keys()):
                    ...
                    if not np.all(np.array(meas[direction][region]) == 0.0):
                        ...
                        arr_sim = np.array(
                            sim[direction][region][start + pfd :]
                        ).tolist()
                        data["simulated_active"][direction] += arr_sim
                        data["measured_active"][direction] += np.array(
                            meas[direction][region][pfd : pfd + len(arr_sim)]
                        ).tolist()

                        data["simulated_passive"][direction] += np.array(
                            sim[direction][region][start : pfd + start]
                        ).tolist()
                        data["measured_passive"][direction] += np.array(
                            meas[direction][region][:pfd]
                        ).tolist()
        ...
        if split == 0:
            ...
            for phase in ["active", "passive"]:
                for comp in list(strain_comp.keys()):
            ...
            labels = {"passive": "diastolic", "active": "systolic"}
    ...
    def plot_volume_scatter(self, unload=True):
        ...
        for patient_name in list(self._results.keys()):
            ...
            arr = np.array(self._results[patient_name]["simulated_volume"]).tolist()[
                start:
            ]
            simulated += arr
            ...
            measured += np.array(
                self._results[patient_name]["measured_volume"]
            ).tolist()[start:end]

            print(
                "Patient:{}, Simulated: {}, Measured: {}".format(
                    patient_name, len(simulated), len(measured)
                )
            )
    ...
    def plot_gamma(self, data="both"):
        ...
        if data == "both" or "mean":
            if len(keys) == 0 or "gamma_mean" not in self._results[keys[0]]:
                self.compute("gamma_mean")

        if data == "both" or "regional":
            if len(keys) == 0 or "gamma_regional" not in self._results[keys[0]]:
        ...
        for patient_name in list(self._results.keys()):
            ...
            valve_times = (
                None
                if not hasattr(self, "_valve_times")
                else self._valve_times[patient_name]
            )

            if data == "mean" or "both":
                path = "/".join([outdir, patient_name + "_mean"])
                plot.plot_single_mean_gamma(
                    np.transpose(self._results[patient_name]["gamma_mean"]),
                    path,
                    valve_times,
                )
            ...
            if data == "regional" or "both":
                ...
                plot.plot_single_regional_gamma(
                    np.transpose(self._results[patient_name]["gamma_regional"]),
                    path,
                    valve_times,
    ...
    def plot_gamma_synch(self, data="both", groups=None):
        ...
        def interpolate_data(patient_name, mean, regional, labels):
            ...
            if data in ("mean", "both"):
                ...
                f = utils.interpolate_trace_to_valve_times(
                    self._results[patient_name]["gamma_mean"],
            ...
            if data in ("regional", "both"):
                ...
                for g in self._results[patient_name]["gamma_regional"]:
                    f = utils.interpolate_trace_to_valve_times(
                        g, self._valve_times[patient_name], N
                    )
                    gammas.append(f)
        ...
        if groups is None:
            ...
            for k, v in groups.items():
                ...
                for patient_name in v:
                    if patient_name in list(self._results.keys()):
                        interpolate_data(patient_name, mean, regional, labels)
                    else:
    ...
    def plot_gamma_synch_mean(self, data="both", groups=None):
        ...
        if groups is None:
            ...
            plot_data(mean, regional, list(groups.keys()))
    ...
    def plot_emax(self, groups=None):
        ...
        keys = list(self._results.keys())
        if len(keys) == 0 or "emax" not in self._results[keys[0]]:
            self.compute("emax")
    ...
    def plot_time_varying_elastance(self):
        ...
        if "simulated_volume" not in self._results[keys[0]]:
            self.compute("volume")
    ...
    def print_geometric_distance_table(self):
        ...
        keys = list(self._results.keys())
        if len(keys) == 0 or "geometric_distance" not in self._results[keys[0]]:
        ...
        tables.print_geometric_distance_table(
            mean_dist, max_dist, labels, self._label_key
        )
    ...
    def plot_geometric_distance(self, groups=None):
        ...
        for patient_name in self._keys:
            ...
            d_int = utils.interpolate_trace_to_valve_times(
                d, self._valve_times[patient_name], N
            )
        ...
        if groups:
            ...
            stds = np.std(dists, 1)
            plot.plot_geometric_distance(
                mean_dist, labels, path, valve_times, stds=stds
            )
    ...
    def print_data_mismatch_table(self):
        ...
        keys = list(self._results.keys())
        if len(keys) == 0 or "data_mismatch" not in self._results[keys[0]]:
    ...
    def plot_cardiac_work(self, case="full", work_pair="SE"):
        ...
        if len(keys) == 0 or key not in self._results[keys[0]]:
            try:
        ...
        for key in self._keys:
            ...
            for r in list(self._results[key]["measured_work"].keys()):

                measured_work = np.multiply(
                    self._results[key]["measured_work"][r], scale
                )

                work = []
                labels = []

                assert key_str.format(work_pair, case, r) in self._results[key]

                simulated_work = np.array(
                    self._results[key][key_str.format(work_pair, case, r)]["work"]
                )

                work = [simulated_work]

                path = "/".join(
                    [outdir, "cardiac_work_{}_{}_region_{}".format(work_pair, case, r)]
                )
    ...
    def plot_cardiac_work_scalar(self, workpair="p_gradu_longitudinal"):
        ...
        keys = list(self._results.keys())
        if len(keys) == 0 or "scalar_features" not in self._results[keys[0]]:
            self.compute("mechanical_features")

        if len(keys) == 0 or "simulated_strain" not in self._results[keys[0]]:
            self.compute("strain")

        if len(keys) == 0 or "meausured_work" not in self._results[keys[0]]:
        ...
        for key in self._keys:
            ...
            for r in list(self._results[key]["measured_work"].keys()):
                ...
                if workpair == "p_gradu_longitudinal":
                    ...
                    simulated_work = utils.compute_cardiac_work_echo(
                        pressure, strain, flip=True
                    )
                ...
                #     simulated_work = utils.compute_cardiac_work_echo(pressure, strain, flip=True)
                    ...
                    strain = self._results[key]["features_scalar"][strain_keystr][
                        str(r)
                    ]
                    stress = self._results[key]["features_scalar"][stress_keystr][
                        str(r)
                    ]

                    simulated_work = utils.compute_cardiac_work_echo(
                        stress, strain, flip=False
                    )
    ...
    def snap_shots(self, feature="", feature_space="DG_0"):
        ...
        if feature != "":
            all_features = list(self._features[self._keys[0]].keys())
        ...
        for patient_name in self._keys:
            ...
            params["Patient_parameters"]["patient"] = patient_name
            params["Patient_parameters"]["mesh_path"] = self._mesh_path.format(
                patient_name
            )
            params["Patient_parameters"]["pressure_path"] = self._pressure_path.format(
                patient_name
            )
