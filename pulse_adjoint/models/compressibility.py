...
def get_compressibility(parameters):
    ...
    if "compressibility" not in parameters:
        return Compressibility.Incompressible(parameters)

    assert "type" in parameters["compressibility"]
...
class Compressibility(object):
    class Incompressible(object):
        def __init__(self, parameters):
            ...
            element = (
                "taylor_hood"
                if "element_type" not in parameters
            ...
            if DOLFIN_VERSION_MAJOR > 1.6:
                ...
                if element == "taylor_hood":
                    ...
                    logger.warning(
                        "MINI elements are experimental. " "Things might fail."
                    )
                ...
                if element == "mini":
                    logger.warning(
                        "mini elements are not supported "
        ...
        def get_displacement(self, name, annotate=True):
            ...
            fa.assign(u, self.w.split()[0], annotate=annotate)
            return u
        ...
        def get_hydrostatic_pressue(self, name="p", annotate=True):
            ...
            fa.assign(p, self.w.split()[1], annotate=annotate)
            return p
    ...
    class StabalizedIncompressible(Incompressible):
        ...
        def __init__(self, parameters):
            ...
            if "lambda" in parameters["compressibility"]:
                self.lamda = Constant(
                    parameters["compressibility"]["lambda"], name="incomp_penalty"
                )
            else:
    ...
    class ThreeFieldLV(Incompressible):
        ...
        def get_pinn(self, annotate=False, name="p_inner"):
            ...
            fa = FunctionAssigner(V, D)
            u = Function(V, name=name)
            fa.assign(u, self.w.split()[2], annotate=annotate)
            return u
